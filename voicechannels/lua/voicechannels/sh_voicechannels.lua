...
voicechannels.NewChannel = function( chanid, adminid, chanpass )
	...
	if chanid and adminid then
		...
		if newid == "global" then
			
			return false, "You cannot recreate the Global Channel!"
...
voicechannels.RemoveChannel = function( chanid )
	...
	if chanid then
		...
		if newid == "global" then
			...
			return false, "Can't remove the global channel..."
		
		end
		
		if not voicechannels.channels[newid] then
...
function plymeta:VoiceChannels_GetChannel()
	...
	if IsValid( self ) then
		...
		return voicechannels.users[self:SteamID()] or "global"
